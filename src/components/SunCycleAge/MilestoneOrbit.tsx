...
const MilestoneOrbit: React.FC<MilestoneOrbitProps> = ({ centerX = 100, centerY = 70, radius = 42 }) => (
  ...
  >
    {/* Orbit path */}
    <circle cx={centerX} cy={centerY} r={radius} stroke="currentColor" strokeWidth="2" className="text-gray-400 dark:text-gray-600" />
    ...
    <defs>
      ...
      <filter id="sun-glow-dark" x="-40%" y="-40%" width="180%" height="180%">
        <feGaussianBlur stdDeviation="10" result="coloredBlur" />
        <feFlood floodColor="#a5b4fc" floodOpacity="0.5" result="glowColor" />
        <feComposite in="glowColor" in2="coloredBlur" operator="in" result="glow" />
        <feMerge>
          <feMergeNode in="glow" />
          <feMergeNode in="SourceGraphic" />
    ...
    <g style={{ transformOrigin: `${centerX}px ${centerY}px`, animation: 'orbit 4s linear infinite' }}>
      ...
      <text x={centerX + radius} y={centerY + 24} textAnchor="middle" fontSize="14" className="fill-gray-500 dark:fill-gray-300 font-mono">You</text>
    </g>
    <style>{`
      @keyframes orbit {
